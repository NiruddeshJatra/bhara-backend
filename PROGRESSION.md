...
**2026-06-11 — full perf audit (PERFORMANCE_AUDIT.md + PERFORMANCE_PLAN.md), 9 optimizations, 5 security fixes**

- **Perf, DB layer**: `CONN_MAX_AGE=60` + `CONN_HEALTH_CHECKS` in production (persistent Postgres connections); 5 composite indexes via 3 migrations — `Rental(product, status, start_date, end_date)` (overlap guard), `Rental(renter, status)` / `Rental(owner, status)`, `Product(status, -created_at)` (listing page), `Review(product, direction)`.
- **Perf, query paths**: `get_blocked_dates(product, start=None, end=None)` — window args push overlap filter into SQL instead of expanding a product's whole rental history into a Python set; rental create passes its window. `distinct=True` on ProductFilter `min_price`/`max_price`/`duration_unit` (products with several matching tiers duplicated in results — also a correctness bug). Rental list endpoints no longer prefetch `payment_records` (only `retrieve` does). `Rental._apply` uses `save(update_fields=...)`; auto-reject uses `bulk_update`. Product retrieve drops `refresh_from_db` (in-memory increment for display).
- **Pagination (response-shape change, user-approved)**: NEW `core/pagination.py` — `StandardResultsSetPagination` + `paginated_success_response` helper. Applied to `my-rentals`, `my-listings-rentals`, `GET /api/reviews/`, `GET /api/reviews/pending/`; listings imports the shared class. These four endpoints now return `data = {count, next, previous, results}` — frontend must adapt.
